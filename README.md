...
In order for Pre-ES6 environments to test if a value containing a forEach() method is a `Set`, either `Object.prototype.toString.call(mySet)` must return `"[object Set]"` (ES6+ behavior) or `mySet instanceof Set` must return true. In order for the `instanceof` check to work across global environments, `eval()` must be permitted or an `EvalError` will be thrown but only if `myObj` has a .add property, does not have a `[Symbol.iterator]()` method, and `myObj instanceof Object` returns false which is typically only when `myObj` was created via a foreign global environment's constructor.
